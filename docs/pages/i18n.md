...
`Engine.I18nScriptTag` emits the loader script tag that, in the browser, fetches
and installs translations for a profile. It uses the **public client key** (safe
to embed in HTML) — never the server key:
...
Pair it with `BootstrapScriptTag` (see [Advanced](advanced.md) / SSR) so the
browser has flags AND translations on first paint:

```go
user := shipeasy.User{"user_id": "u_123"}
head := eng.BootstrapScriptTag(user, shipeasy.BootstrapTagOptions{AnonID: anonID}) +
    eng.I18nScriptTag(clientKey, "{{PROFILE}}", shipeasy.BootstrapTagOptions{})
```
