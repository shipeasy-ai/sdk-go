/*
Shipeasy Admin API

REST API for managing feature gates, experiments, configs, universes, and killswitches in a Shipeasy project. Authenticate with an admin SDK key (`Authorization: Bearer sdk_admin_…`) and scope every request to a project via the `X-Project-Id` header.  Mint admin keys via `POST /api/admin/keys` with `type: \"admin\"`. Keys expire after 90 days; rotate with the `revoke` action.

API version: 2.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package admin

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the CreateConfigRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CreateConfigRequest{}

// CreateConfigRequest Body for `POST /api/admin/configs`. `name` + `schema` required. Per-env `dev`/`staging`/`prod` values are published to that env at version 1 (overriding `value`).
type CreateConfigRequest struct {
	// Stable config/killswitch key in `folder.name` form (two lowercase segments separated by a dot, e.g. `pricing.tiers`). Immutable after create.
	Name string `json:"name" validate:"regexp=^(?:_default|[a-z0-9](?:[a-z0-9_-]*[a-z0-9])?)\\\\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9])?$"`
	// Optional free-form description shown in the dashboard. Max 512 chars.
	Description *string `json:"description,omitempty"`
	// Optional folder name grouping items in the dashboard. Alphanumeric, `_` or `-` (no `/`). Part of the SDK lookup key (`<folder>/<name>`).
	Folder NullableString `json:"folder,omitempty" validate:"regexp=^[a-zA-Z0-9_-]+$"`
	// JSON Schema (draft 2020-12) describing the shape of the config value. Top-level `type` must be `'object'`; every published value is validated against this schema.
	Schema map[string]interface{} `json:"schema"`
	// Initial config value. Either a single JSON object applied to every env, or a `{ env: value }` map seeding per-env values. Must match `schema`. Defaults to `{}` on every env when omitted.
	Value map[string]interface{} `json:"value,omitempty"`
	// Seed the **dev** env's initial value (version 1), overriding `value` for dev. Published immediately. Must match `schema`.
	Dev map[string]interface{} `json:"dev,omitempty"`
	// Seed the **staging** env's initial value (version 1), overriding `value` for staging. Published immediately. Must match `schema`.
	Staging map[string]interface{} `json:"staging,omitempty"`
	// Seed the **prod** env's initial value (version 1), overriding `value` for prod. Published immediately. Must match `schema`.
	Prod map[string]interface{} `json:"prod,omitempty"`
}

type _CreateConfigRequest CreateConfigRequest

// NewCreateConfigRequest instantiates a new CreateConfigRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCreateConfigRequest(name string, schema map[string]interface{}) *CreateConfigRequest {
	this := CreateConfigRequest{}
	this.Name = name
	this.Schema = schema
	return &this
}

// NewCreateConfigRequestWithDefaults instantiates a new CreateConfigRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCreateConfigRequestWithDefaults() *CreateConfigRequest {
	this := CreateConfigRequest{}
	return &this
}

// GetName returns the Name field value
func (o *CreateConfigRequest) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *CreateConfigRequest) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *CreateConfigRequest) SetName(v string) {
	o.Name = v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *CreateConfigRequest) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateConfigRequest) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *CreateConfigRequest) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *CreateConfigRequest) SetDescription(v string) {
	o.Description = &v
}

// GetFolder returns the Folder field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CreateConfigRequest) GetFolder() string {
	if o == nil || IsNil(o.Folder.Get()) {
		var ret string
		return ret
	}
	return *o.Folder.Get()
}

// GetFolderOk returns a tuple with the Folder field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CreateConfigRequest) GetFolderOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Folder.Get(), o.Folder.IsSet()
}

// HasFolder returns a boolean if a field has been set.
func (o *CreateConfigRequest) HasFolder() bool {
	if o != nil && o.Folder.IsSet() {
		return true
	}

	return false
}

// SetFolder gets a reference to the given NullableString and assigns it to the Folder field.
func (o *CreateConfigRequest) SetFolder(v string) {
	o.Folder.Set(&v)
}
// SetFolderNil sets the value for Folder to be an explicit nil
func (o *CreateConfigRequest) SetFolderNil() {
	o.Folder.Set(nil)
}

// UnsetFolder ensures that no value is present for Folder, not even an explicit nil
func (o *CreateConfigRequest) UnsetFolder() {
	o.Folder.Unset()
}

// GetSchema returns the Schema field value
func (o *CreateConfigRequest) GetSchema() map[string]interface{} {
	if o == nil {
		var ret map[string]interface{}
		return ret
	}

	return o.Schema
}

// GetSchemaOk returns a tuple with the Schema field value
// and a boolean to check if the value has been set.
func (o *CreateConfigRequest) GetSchemaOk() (map[string]interface{}, bool) {
	if o == nil {
		return map[string]interface{}{}, false
	}
	return o.Schema, true
}

// SetSchema sets field value
func (o *CreateConfigRequest) SetSchema(v map[string]interface{}) {
	o.Schema = v
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *CreateConfigRequest) GetValue() map[string]interface{} {
	if o == nil || IsNil(o.Value) {
		var ret map[string]interface{}
		return ret
	}
	return o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateConfigRequest) GetValueOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.Value) {
		return map[string]interface{}{}, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *CreateConfigRequest) HasValue() bool {
	if o != nil && !IsNil(o.Value) {
		return true
	}

	return false
}

// SetValue gets a reference to the given map[string]interface{} and assigns it to the Value field.
func (o *CreateConfigRequest) SetValue(v map[string]interface{}) {
	o.Value = v
}

// GetDev returns the Dev field value if set, zero value otherwise.
func (o *CreateConfigRequest) GetDev() map[string]interface{} {
	if o == nil || IsNil(o.Dev) {
		var ret map[string]interface{}
		return ret
	}
	return o.Dev
}

// GetDevOk returns a tuple with the Dev field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateConfigRequest) GetDevOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.Dev) {
		return map[string]interface{}{}, false
	}
	return o.Dev, true
}

// HasDev returns a boolean if a field has been set.
func (o *CreateConfigRequest) HasDev() bool {
	if o != nil && !IsNil(o.Dev) {
		return true
	}

	return false
}

// SetDev gets a reference to the given map[string]interface{} and assigns it to the Dev field.
func (o *CreateConfigRequest) SetDev(v map[string]interface{}) {
	o.Dev = v
}

// GetStaging returns the Staging field value if set, zero value otherwise.
func (o *CreateConfigRequest) GetStaging() map[string]interface{} {
	if o == nil || IsNil(o.Staging) {
		var ret map[string]interface{}
		return ret
	}
	return o.Staging
}

// GetStagingOk returns a tuple with the Staging field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateConfigRequest) GetStagingOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.Staging) {
		return map[string]interface{}{}, false
	}
	return o.Staging, true
}

// HasStaging returns a boolean if a field has been set.
func (o *CreateConfigRequest) HasStaging() bool {
	if o != nil && !IsNil(o.Staging) {
		return true
	}

	return false
}

// SetStaging gets a reference to the given map[string]interface{} and assigns it to the Staging field.
func (o *CreateConfigRequest) SetStaging(v map[string]interface{}) {
	o.Staging = v
}

// GetProd returns the Prod field value if set, zero value otherwise.
func (o *CreateConfigRequest) GetProd() map[string]interface{} {
	if o == nil || IsNil(o.Prod) {
		var ret map[string]interface{}
		return ret
	}
	return o.Prod
}

// GetProdOk returns a tuple with the Prod field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateConfigRequest) GetProdOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.Prod) {
		return map[string]interface{}{}, false
	}
	return o.Prod, true
}

// HasProd returns a boolean if a field has been set.
func (o *CreateConfigRequest) HasProd() bool {
	if o != nil && !IsNil(o.Prod) {
		return true
	}

	return false
}

// SetProd gets a reference to the given map[string]interface{} and assigns it to the Prod field.
func (o *CreateConfigRequest) SetProd(v map[string]interface{}) {
	o.Prod = v
}

func (o CreateConfigRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CreateConfigRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["name"] = o.Name
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if o.Folder.IsSet() {
		toSerialize["folder"] = o.Folder.Get()
	}
	toSerialize["schema"] = o.Schema
	if !IsNil(o.Value) {
		toSerialize["value"] = o.Value
	}
	if !IsNil(o.Dev) {
		toSerialize["dev"] = o.Dev
	}
	if !IsNil(o.Staging) {
		toSerialize["staging"] = o.Staging
	}
	if !IsNil(o.Prod) {
		toSerialize["prod"] = o.Prod
	}
	return toSerialize, nil
}

func (o *CreateConfigRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"name",
		"schema",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varCreateConfigRequest := _CreateConfigRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varCreateConfigRequest)

	if err != nil {
		return err
	}

	*o = CreateConfigRequest(varCreateConfigRequest)

	return err
}

type NullableCreateConfigRequest struct {
	value *CreateConfigRequest
	isSet bool
}

func (v NullableCreateConfigRequest) Get() *CreateConfigRequest {
	return v.value
}

func (v *NullableCreateConfigRequest) Set(val *CreateConfigRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableCreateConfigRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableCreateConfigRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCreateConfigRequest(val *CreateConfigRequest) *NullableCreateConfigRequest {
	return &NullableCreateConfigRequest{value: val, isSet: true}
}

func (v NullableCreateConfigRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCreateConfigRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


