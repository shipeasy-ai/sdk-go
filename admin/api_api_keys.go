...
func (r ApiListKeysRequest) Q(q string) ApiListKeysRequest {
	r.q = &q
	return r
}
...
// Project the request operates on. Optional — defaults to the project the SDK key belongs to; pass it only to scope a multi-project key (the generated client sets it once from its configuration, so per-call callers never thread it).
func (r ApiUpdateKeyRequest) XProjectId(xProjectId string) ApiUpdateKeyRequest {
	r.xProjectId = &xProjectId
	return r
}

func (r ApiUpdateKeyRequest) Execute() (*KeyRecord, *http.Response, error) {
	return r.ApiService.UpdateKeyExecute(r)
}

/*
UpdateKey Update an API key's label
...
*/
func (a *APIKeysAPIService) UpdateKey(ctx context.Context, id string) ApiUpdateKeyRequest {
	return ApiUpdateKeyRequest{
		ApiService: a,
		ctx: ctx,
		id: id,
...
// Execute executes the request
//  @return KeyRecord
func (a *APIKeysAPIService) UpdateKeyExecute(r ApiUpdateKeyRequest) (*KeyRecord, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPatch
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *KeyRecord
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "APIKeysAPIService.UpdateKey")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/admin/keys/{id}"
	localVarPath = strings.Replace(localVarPath, "{"+"id"+"}", url.PathEscape(parameterValueToString(r.id, "id")), -1)
